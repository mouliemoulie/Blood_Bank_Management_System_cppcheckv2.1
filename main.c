...
#define BMS_MAX_DONATION_APPOINTMENTS   (100U)
#define BMS_APPOINTMENTS_FILE            "data/donation_appointments.dat"
...
typedef enum
{
    BMS_APPOINTMENT_PENDING = 1,
    BMS_APPOINTMENT_APPROVED,
    BMS_APPOINTMENT_COMPLETED,
    BMS_APPOINTMENT_CANCELLED
} BmsAppointmentStatus_t;

typedef struct
{
    uint32_t appointmentId;
    BmsUserId_t donorUserId;
    BmsDonorId_t donorId;
    BmsHospitalId_t centerHospitalId;
    BmsDate_t preferredDate;
    BmsAppointmentStatus_t status;
} BmsDonationAppointment_t;
...
typedef struct
{
    ...
    BmsDonationAppointment_t appointments[BMS_MAX_DONATION_APPOINTMENTS];
    uint32_t appointmentCount;
} BmsApplication_t;
...
static BmsUiOperationResult_t BookDonationAppointment(BmsApplication_t *application, const BmsUser_t *user);
static void ViewMyAppointments(const BmsApplication_t *application, const BmsUser_t *user);
static BmsStatus_t LoadAppointments(BmsApplication_t *application);
static BmsStatus_t SaveAppointments(const BmsApplication_t *application);
...
static BmsStatus_t ApplicationInitialize(BmsApplication_t *application)
{
    ...
    if (application != NULL)
    {
        ...
        if (status == BMS_STATUS_OK)
        {
            ...
            loadStatus = LoadAppointments(application);
            if ((loadStatus != BMS_STATUS_OK) &&
                (loadStatus != BMS_STATUS_FILE_NOT_FOUND))
            {
                PrintStatus("Load appointment data", loadStatus);
            }
        }
    }
...
static BmsStatus_t ApplicationSave(BmsApplication_t *application)
{
    ...
    if (application != NULL)
    {
        ...
        status = SaveAppointments(application);
        if (status != BMS_STATUS_OK)
        {
            PrintStatus("Save appointment data", status);
            overallStatus = status;
        }
    }
                                           ...
                                           const BmsUser_t *user)
{
    ...
    while (!finished)
    {
        ...
        (void)printf("3. Volunteer for donation (book appointment)\n");
        (void)printf("4. View donation appointments\n");
        (void)printf("5. View donation history\n");
        (void)printf("6. View donation camps\n");
        (void)printf("7. View notifications\n");
        (void)printf("8. Change password\n");
        (void)printf("9. Logout\n");
        ...
        switch (choice)
        {
            ...
            case 3U: HANDLE_OPERATION_RESULT(BookDonationAppointment(application, user)); break;
            case 4U: ViewMyAppointments(application, user); break;
            case 5U: ListDonations(application); break;
            case 6U: ViewDonationCamps(application); break;
            case 7U: ViewNotifications(application, user); break;
            case 8U: HANDLE_OPERATION_RESULT(ChangePassword(application, user)); break;
            case 9U: finished = true; break;
            default:
                (void)printf("Invalid choice. Enter 1 to 9.\n");
                break;
        }
    }
...
static BmsUiOperationResult_t BookDonationAppointment(BmsApplication_t *application,
                                                       const BmsUser_t *user)
{
    BmsDonationAppointment_t appointment;
    uint32_t field = 0U;
    ...
    if (application->appointmentCount >= BMS_MAX_DONATION_APPOINTMENTS)
    {
        (void)printf("Appointment storage is full.\n");
        return BMS_UI_OPERATION_DONE;
    }

    (void)memset(&appointment, 0, sizeof(appointment));
    appointment.appointmentId = application->appointmentCount + 1U;
    appointment.donorUserId = user->userId;
    appointment.status = BMS_APPOINTMENT_PENDING;

    PrintTitle("VOLUNTEER FOR BLOOD DONATION");
    PrintNavigationHelp();
    ...
    while (field < 3U)
    {
        ...
        {
            case 0U:
                inputResult = UiReadUint32("Donor profile ID: ", &appointment.donorId);
                break;
            case 1U:
                inputResult = UiReadUint32("Donation center hospital ID: ",
                                           &appointment.centerHospitalId);
                break;
            case 2U:
                inputResult = UiReadDate("Preferred date (YYYY-MM-DD): ",
    ...
    application->appointments[application->appointmentCount] = appointment;
    ++application->appointmentCount;
    PrintStatus("Book donation appointment", SaveAppointments(application));
    (void)printf("Appointment ID: %lu | Status: Pending\n",
                 (unsigned long)appointment.appointmentId);
    return BMS_UI_OPERATION_DONE;
}

static void ViewMyAppointments(const BmsApplication_t *application,
                               const BmsUser_t *user)
{
    if ((application != NULL) && (user != NULL))
    {
        bool found = false;
        PrintTitle("MY DONATION APPOINTMENTS");
        for (uint32_t index = 0U; index < application->appointmentCount; ++index)
        {
            const BmsDonationAppointment_t *appointment =
                &application->appointments[index];
            if (appointment->donorUserId == user->userId)
            {
                found = true;
                (void)printf("ID:%lu | Donor:%lu | Center:%lu | %04u-%02u-%02u | Status:%u\n",
                             (unsigned long)appointment->appointmentId,
                             (unsigned long)appointment->donorId,
                             (unsigned long)appointment->centerHospitalId,
                             appointment->preferredDate.year,
                             appointment->preferredDate.month,
                             appointment->preferredDate.day,
                             (unsigned int)appointment->status);
            }
        }
        if (!found)
        {
            (void)printf("No appointments found.\n");
        }
    }
}
    ...
    while (field < 4U)
    {
        ...
        switch (field)
        {
            ...
            case 2U:
                ...
                break;
            case 3U:
                result = UiReadDate("Expiry date (YYYY-MM-DD): ",
                                    &record.expiryDate);
                break;
            default:
                result = BMS_UI_INVALID;
    ...
    while (field < 5U)
    {
        ...
        switch (field)
        {
            ...
            case 3U:
                ...
                break;
            case 4U:
                result = UiReadUint32("Collection hospital ID: ",
...
static BmsUiOperationResult_t CreateEmergencyAlert(BmsApplication_t *application,const BmsUser_t *user)
{BmsEmergencyAlert_t a;uint32_t field=0U;if((application==NULL)||(user==NULL)||user->hospitalId==0U){(void)printf("Register/link your hospital first.\n");return BMS_UI_OPERATION_DONE;}(void)memset(&a,0,sizeof(a));a.alertId=NextAlertId(application);a.sourceHospitalId=user->hospitalId;a.createdByUserId=user->userId;a.priority=BMS_PRIORITY_EMERGENCY;PrintTitle("CREATE EMERGENCY ALERT");(void)printf("Alert ID: ALT%06lu | Source Hospital: HOS%06lu (auto-linked)\n",(unsigned long)a.alertId,(unsigned long)a.sourceHospitalId);while(field<3U){BmsUiInputResult_t r=BMS_UI_INVALID;switch(field){case 0U:r=UiReadBloodGroup(&a.bloodGroup);break;case 1U:r=UiReadUint32("Required units: ",&a.requiredUnits);break;case 2U:r=UiReadText("Emergency reason/message: ",a.message,sizeof(a.message));break;default:break;}HANDLE_FIELD_RESULT(r,field);}PrintStatus("Create emergency alert",EmergencyAlertManagementCreate(&application->alerts,&a));{BmsNotification_t n;(void)memset(&n,0,sizeof(n));n.notificationId=a.alertId;n.priority=BMS_PRIORITY_EMERGENCY;n.channel=BMS_NOTIFICATION_CHANNEL_BROADCAST;(void)snprintf(n.message,sizeof(n.message),"Emergency alert ALT%06lu from HOS%06lu: %u units %s - %.120s",(unsigned long)a.alertId,(unsigned long)a.sourceHospitalId,a.requiredUnits,BloodGroupToString(a.bloodGroup),a.message);(void)NotificationManagementEnqueue(&application->notifications,&n);}return BMS_UI_OPERATION_DONE;}
...
static BmsStatus_t LoadAppointments(BmsApplication_t *application)
{
    ...
    if (application != NULL)
    {
        file = fopen(BMS_APPOINTMENTS_FILE, "rb");
        if (file == NULL)
        {
            status = BMS_STATUS_FILE_NOT_FOUND;
        }
        else if (fread(&count, sizeof(count), 1U, file) != 1U)
        {
            status = BMS_STATUS_READ_ERROR;
        }
        else if (count > BMS_MAX_DONATION_APPOINTMENTS)
        {
            status = BMS_STATUS_FILE_CORRUPT;
        }
        else if ((count > 0U) &&
                 (fread(application->appointments,
                        sizeof(BmsDonationAppointment_t), count, file) != count))
        {
            status = BMS_STATUS_READ_ERROR;
        }
        else
        {
            application->appointmentCount = count;
            status = BMS_STATUS_OK;
        }
...
static BmsStatus_t SaveAppointments(const BmsApplication_t *application)
{
    ...
    if (application != NULL)
    {
        file = fopen(BMS_APPOINTMENTS_FILE, "wb");
        if (file == NULL)
        {
            status = BMS_STATUS_FILE_ERROR;
        }
        else if (fwrite(&application->appointmentCount,
                        sizeof(application->appointmentCount), 1U, file) != 1U)
        {
            status = BMS_STATUS_WRITE_ERROR;
        }
        else if ((application->appointmentCount > 0U) &&
                 (fwrite(application->appointments,
                         sizeof(BmsDonationAppointment_t),
                         application->appointmentCount, file) !=
                  application->appointmentCount))
        {
            status = BMS_STATUS_WRITE_ERROR;
        }
